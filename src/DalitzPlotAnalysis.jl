...
export change_basis, cross_basis_cosθ3, cross_basis_cosθ12, cross_basis_s3, cross_basis_z23
...
function cross_basis_cosθ3(s1, cosθ23, m1sq, m2sq, m3sq, s)
    ...
    #  λλ = λ(s, m1sq, s1)*λ(s, m3sq, s3)
     sqrtλλ = let m1 = sqrt(m1sq), m3 = sqrt(m3sq), sqrts = sqrt(s)
          sqrt((sqrts+m1)^2-s1) * sqrt((sqrts-m1)^2-s1) *
             sqrt((sqrts+m3)^2-s3) * sqrt((sqrts-m3)^2-s3)
     end
    #  (typeof(s1) == :Float64) && (λλ < 0) && error("λλ < 0, check masses")
    (sqrtλλ ≈ zero(s)) && error("λλ ≈ 0, check masses")
     # directly get the angle
     return (2s*(m1sq+m3sq-s2)+(s+m1sq-s1)*(s+m3sq-s3)) / sqrtλλ
end

function cross_basis_cosθ3(s1, cosθ23, m1sq, m2sq, m3sq, s, s3)
    ...
    #  (typeof(s1) == :Float64) && (λλ < 0) && error("λλ < 0, check masses")
    (sqrtλλ ≈ zero(s)) && error("λλ ≈ 0, check masses, s1 = $s1")
     # directly get the angle
     return (2s*(m1sq+m3sq-s2)+(s+m1sq-s1)*(s+m3sq-s3))/sqrtλλ
end
...
function cross_basis_s3(s1, cosθ23, m1sq, m2sq, m3sq, s)
     # calculate s3 = (p1+p2)^2 in (23) frame
     m1 = sqrt(m1sq); sqrts = sqrt(s)
     sqrtλs1 = sqrt((sqrts-m1)^2-s1) * sqrt((sqrts+m1)^2-s1);
     return m1sq + m2sq +
          ( (s1+m2sq-m3sq) * (s-m1sq-s1) +
            sqrt(s1-(sqrt(m2sq)+sqrt(m3sq))^2) * sqrt(s1-(sqrt(m2sq)-sqrt(m3sq))^2) *
                cosθ23 * sqrtλs1 ) / (2*s1)
end

function cross_basis_z23(s1, s3, m1sq, m2sq, m3sq, s)
     # calculate cosθ23: angle of 2 in the (23) rest frame
     m1 = sqrt(m1sq); sqrts = sqrt(s)
     sqrtλs1 = sqrt((sqrts-m1)^2-s1) * sqrt((sqrts+m1)^2-s1);
     return (2*s1*(s3 - m1sq - m2sq) - (s1+m2sq-m3sq) * (s-m1sq-s1)) /
            ( sqrt(s1-(sqrt(m2sq)+sqrt(m3sq))^2) * sqrt(s1-(sqrt(m2sq)-sqrt(m3sq))^2) *
              sqrtλs1 )
end
...
function cross_basis_cosθ12(s1, cosθ23, m1sq, m2sq, m3sq, s)
     s3 = cross_basis_s3(s1, cosθ23, m1sq, m2sq, m3sq, s)
     m1 = sqrt(m1sq); m2 = sqrt(m2sq); m3 = sqrt(m3sq); sqrts = sqrt(s)

     cosθ12_n = 2s3*(m2sq+m3sq -s1) + (s3+m2sq-m1sq) * (s-m3sq-s3);
     sqrtλλ = sqrt(s3-(m1+m2)^2) * sqrt(s3-(m1-m2)^2) *
              sqrt((sqrts-m3)^2-s3) * sqrt((sqrts+m3)^2-s3);
     # cosθ12 = cosθ12_d ≈ 0.0+0.0im ? 2.0*rand()-1.0*one(s1) : cosθ12_n/cosθ12_d;
     # cosθ12 = 2.0*rand()-1.0*one(s1)
     (sqrtλλ ≈ zero(s)) && error("λλ == 0: check your masses, pars: s1 = $s1, cosθ23 = $cosθ23")
     cosθ12 = cosθ12_n/sqrtλλ

     return cosθ12
end

function cross_basis_cosθ12(s1, cosθ23, m1sq, m2sq, m3sq, s, s3)
     m1 = sqrt(m1sq); m2 = sqrt(m2sq); m3 = sqrt(m3sq); sqrts = sqrt(s)
     cosθ12_n = 2s3*(m2sq+m3sq -s1) + (s3+m2sq-m1sq) * (s-m3sq-s3);
     sqrtλλ = sqrt(s3-(m1+m2)^2) * sqrt(s3-(m1-m2)^2) *
              sqrt((sqrts-m3)^2-s3) * sqrt((sqrts+m3)^2-s3);
...
for tp in [:Float64, :(Complex{Float64})]
    @eval function change_basis(s1::$(tp), cosθ1::Float64, ϕ1::Float64, cosθ23::Float64, ϕ23::Float64,
        ...
        # calculate s3 in (23) frame
        m1 = sqrt(m1sq); sqrts = sqrt(s)
        sqrtλs1 = sqrt((sqrts-m1)^2-s1)*sqrt((sqrts+m1)^2-s1)/(2*sqrt(s1));
        s3 = m1sq + m2sq +
        # 2*(  E2 * E1 -
            2*( (s1 + m2sq - m3sq)/(2*sqrt(s1)) * (s-m1sq-s1)/(2*sqrt(s1)) -
            # |p2|*cos(theta23) * (-|p1|)
            sqrt(λ(s1, m2sq, m3sq)/(4*s1))*cosθ23 * (-sqrtλs1) );
        # caluclate p3 in (23) frame
        p23bu = sqrt(λ(s1, m2sq, m3sq)/(4*s1));
        ...
        cosθ12_n = m2sq + m3sq + 2* (s3+m2sq-m1sq)/(2*sqrt(s3)) * (s-m3sq-s3)/(2*sqrt(s3)) - s1;
        m3 = sqrt(m3sq)
        sqrtλs3 = sqrt((sqrts-m3)^2-s3)*sqrt((sqrts+m3)^2-s3)
        cosθ12_d = 2 * sqrt(λ(s3, m1sq, m2sq)/(4*(s3))) * sqrtλs3 / (2*sqrt(s3));
        cosθ12 = cosθ12_d ≈ 0.0+0.0im ? 2.0*rand()-1.0*one(s1) : cosθ12_n/cosθ12_d;
